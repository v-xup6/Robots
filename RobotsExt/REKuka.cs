...
{
    public class DefJoints : GH_Component
    {
        public DefJoints()
    ...
    public class KukaUDPsender : GH_Component
    {
        public KukaUDPsender() : base("Remote KUKA", "Remote", "Interop with KUKA controller through a network. [Requires KukaVarProxy]", "Robots", "Components") { }
        ...
        public override void AddedToDocument(GH_Document ghDoc)
        {
            ghDoc.ObjectsDeleted += CloseTCPIP;
            base.AddedToDocument(ghDoc);
        }
        public override void AppendAdditionalMenuItems(ToolStripDropDown menu)
        {
            base.AppendAdditionalMenuItems(menu);
            Menu_AppendItem(menu, "Ping IP", (s, e) =>
            {
                long time = 0;
                string ip = Params.Input[1].VolatileData.DataDescription(false, false).Replace("\r", "").Replace("\n", "").Replace(" ", "");
                if (Ping(ip, ref time))
                {
                    MessageBox.Show("Time=" + time + "ms");
                }
                else MessageBox.Show(ip + "\r\nNot Found");
            }, true);
        }
        protected override void RegisterInputParams(GH_InputParamManager pManager)
        {
            //(ASCII)"$AXIS_ACT"=="0004000C00000924415849535F414354"(HEX)
            pManager.AddTextParameter("Command", "C", "Command to send, will query joint angle if nothing connected", GH_ParamAccess.item, "$AXIS_ACT");
            pManager.AddTextParameter("IP Address", "IP", "IP address of controller", GH_ParamAccess.item, "172.31.1.147");
        ...
        static readonly int port = 7000;
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            byte write = 0;//Write and read array??
            string ip = string.Empty, input = string.Empty;
            bool run = false;
            DA.GetData("IP Address", ref ip);
            DA.GetData("Command", ref input);
            DA.GetData("Execute Program", ref run);

            if (run)
            {
                List<string> log = new List<string>();
                byte[] varName, nameL, varVal = null, varL = null;
                string[] cmd = input.Split(new char[] { '=' }, StringSplitOptions.RemoveEmptyEntries);

                if (!(cmd.Length == 1 || cmd.Length == 2) || !cmd[0].StartsWith("$"))
                {
                    AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "Can't parse command");
                    log.Add($"{input} is a invalid or unsupported command");
                ...
                {
                    write = 1;
                    varVal = Encoding.ASCII.GetBytes(cmd[1].Replace(" ", ""));
                    varL = BitConverter.IsLittleEndian ? 
                        BitConverter.GetBytes((UInt16)varVal.Length).Reverse().ToArray() : 
                        BitConverter.GetBytes((UInt16)varVal.Length);
                }
                varName = Encoding.ASCII.GetBytes(cmd[0].Replace(" ", ""));
                nameL = BitConverter.IsLittleEndian ? 
                    BitConverter.GetBytes((UInt16)varName.Length).Reverse().ToArray() : 
                    BitConverter.GetBytes((UInt16)varName.Length);

                List<byte> msg = new List<byte>();
                UInt16 timestamp = Convert.ToUInt16(DateTime.Now.Minute * 100 + DateTime.Now.Second);
                Array.ForEach(BitConverter.IsLittleEndian ?
                    BitConverter.GetBytes(timestamp).Reverse().ToArray() :
                    BitConverter.GetBytes(timestamp), id => msg.Add(id)
                    );
                UInt16 reqLength = Convert.ToUInt16(write == 1 ?
                    (1 + 2 + varName.Length + 2 + varVal.Length) :
                    (1 + 2 + varName.Length));
                Array.ForEach(BitConverter.IsLittleEndian ?
                    BitConverter.GetBytes(reqLength).Reverse().ToArray() :
                    BitConverter.GetBytes(reqLength), id => msg.Add(id)
                    );
                msg.Add(write);
                Array.ForEach(nameL, b => msg.Add(b));
                Array.ForEach(varName, b => msg.Add(b));
                if (write == 1)
                {
                    Array.ForEach(varL, b => msg.Add(b));
                    Array.ForEach(varVal, b => msg.Add(b));
                }
                byte[] tcpmsg = msg.ToArray();
#if(DEBUG)
                log.Add("Sent: " + B2Hex(tcpmsg));
#endif
                try
                {
                    if (!client.Connected)
                    ...
                    responseData = Encoding.ASCII.GetString(response, 0, bytes);
                    log.Add("Response: " + responseData);
                }
                catch (Exception e)
                {
        ...
        bool Ping(string IPv4Address, ref long Elapsed)
        {
            Ping p = new Ping();
            PingReply r = p.Send(IPv4Address);
            Elapsed = r.RoundtripTime;
            if (r.Status == IPStatus.Success) return true;
            else return false;
        }
        void CloseTCPIP(object sender, GH_DocObjectEventArgs e)
        {
...
#if (DEBUG)
        static string B2Hex(byte[] bytes)
        {
